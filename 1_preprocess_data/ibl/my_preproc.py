...
if __name__ == '__main__':
    ...
    for filename in os.listdir(data_path):
        if filename.endswith("_trialdata.csv"):
            match = pattern_g1.search(filename)
            if match:
                ...
                print(f'Identified filename: {filename}, subject {subject}')
    ...
    np.savez('partially_processed/subject_list.npz', subject_list)
    ...
    for z, subject in enumerate(subject_list):
        ...
        np.savez(processed_data_path + 'data_by_subj/' + subject + '_unnormalized.npz', subj_unnormalized_input, subj_y)
        # subj_trial_fold_lookup = 
        
        np.savez(processed_data_path + 'data_by_subj/' + subject + '_correct.npz', subj_correct)
    ...
    np.savez(processed_data_path + 'all_subj_concat' + '.npz',
        normalized_input,
        master_y)
    # np.savetxt(processed_data_path + 'all_subj_concat' + '.csv',
    #     normalized_input)
    pd.DataFrame(normalized_input).to_csv(processed_data_path + 'all_subj_concat.csv') # also save as csv for readability
    pd.DataFrame(master_y).to_csv(processed_data_path + 'all_subj_concat_y.csv') # also save master_y as csv
    np.savez(
        processed_data_path + 'all_subj_concat_unnormalized' + '.npz',
        master_input, master_y)
    np.savez(processed_data_path + 'all_subj_concat_correct' + '.npz',
        master_correct)
    np.savez(processed_data_path + 'data_by_subj/' + 'final_subject_list.npz',
        final_subject_list) # ?
    ...
    for subject in subject_start_idx.keys():
        ...
        np.savez(processed_data_path + 'data_by_subj/' + subject + '_processed.npz',
            input, y)
    ...
    np.savez(data_path + 'response_times/data_by_subj/' + subject + '.npz', reaction_time)